...
export const Login: React.FC = () => {
  ...
  // REMOVED: crossDomainLogoutAttempts, showLogoutLoading - 로그아웃 추적 제거
  ...
  // 페이지 로드 시 인증 방법 체크 및 Silent 로그인 시도 - DISABLED FOR CROSS-DOMAIN LOGOUT
  useEffect(() => {
    // 이미 시도했거나 이미 인증된 경우 또는 로그아웃한 경우 또는 강제 새 로그인인 경우 또는 크로스 도메인 로그아웃인 경우 스킵
    if (authAttemptRef.current || isAuthenticated || hasLoggedOutRef.current || forceNewLogin || crossDomainLogoutReason) {
      setSilentLoginAttempting(false);
      if (crossDomainLogoutReason) {
        console.log('🛑 Silent login disabled: Cross-domain logout detected');
      }
      return;
    }
  ...
  }, [navigate, setAuth, crossDomainLogoutReason]); // Cross-domain logout detection
